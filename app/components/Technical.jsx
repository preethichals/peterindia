  ...
  return (
    ...
    <div className="bg-white">
      ...
      <div className="mx-auto grid max-w-2xl grid-cols-1 items-center gap-x-8 gap-y-16 px-4 py-20 sm:px-6 sm:py-20 lg:max-w-7xl lg:grid-cols-2 lg:px-8">
        ...
        <div>
          <h2 className="text-3xl text-center font-bold tracking-tight text-gray-900 sm:text-4xl">Technical Specifications</h2>
          ...
          <dl className="mt-10 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-y-6 lg:gap-x-8">
            {features.map((feature) => (
              <div key={feature.name} className="border-t border-gray-200 pt-4">
                <dt className="font-medium text-gray-900 ms-3">{feature.name}</dt>
                <dd className="mt-2 text-sm text-gray-500 ms-3">{feature.description}</dd>
              </div>
            ))}
          </dl>
        </div>
